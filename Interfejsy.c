...
float readPreassure()
{
	int32_t rawPreassure = (readRegister(0xF7) << 12) | (readRegister(0xF8) << 4) | (readRegister(0xF9)>>4);
...
while (option != 8)
{
	...
	case 1:
	{
		...
		float temp=((float)Temp_konc/340)+36.53; // wzor na temp w stopniach C
	...
	case 7:
	{
		...
		writeRegister(BMP280_REGISTER_CONTROL, 0x57);
